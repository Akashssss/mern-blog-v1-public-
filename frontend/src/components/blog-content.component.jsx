import React from 'react'


const Img = ({ url, caption }) => {

    return <div>
        <img src={url} alt={caption} />
        {
            (caption && caption.length) && <p dangerouslySetInnerHTML={{ __html: caption }}
                className='w-full text-center my-3 md:mb-12  text-base text-dark-grey '></p>
        }
    </div>
}
const Quote = ({ quote, caption }) => {
    return <div className='bg-purple/10 p-3 border-l-4 border-purple'>
        <p className='text-xl leading-10 md:text-2xl ' dangerouslySetInnerHTML={{ __html: quote }}></p>
        {caption.length &&
            <p className='w-full  text-purple text-base '
                dangerouslySetInnerHTML={{ __html: caption }}></p>}
    </div>
}
const List  = ({style , items})=>{
    return  <ol className={`pl-5 ${style=='ordered'?"list-decmal" :"list-disc"}`}>
        {
            items.map((listItem ,key)=>{
                return <li key={key} className='my-4' 
                dangerouslySetInnerHTML={{__html:listItem}}></li>
            })
        }
    </ol>
}

export default function BlogContent({ block }) {
    let { type, data } = block;
    if (type == 'paragraph')
        return <p dangerouslySetInnerHTML={{ __html: data.text }}></p>
    if (type == "header") {
        if (data.level == 4)
            return <h4 className='text-3xl font-bold' dangerouslySetInnerHTML={{ __html: data.text }}></h4>
        if (data.level == 3)
            return <h3 className='text-3xl font-bold' dangerouslySetInnerHTML={{ __html: data.text }}></h3>
        if (data.level == 2)
            return <h2 className='text-3xl font-bold' dangerouslySetInnerHTML={{ __html: data.text }}></h2>
    }
    if (type == 'image') {
        return <Img url={data.file.url} caption={data.caption} />
    }

    if (type == 'quote') {
        return <Quote quote={data.text} caption={data.caption} />
    }
    if(type=='list')
    {
        return <List style= {data.style} items= {data.items}/>
    }
    else {
        return <h1>this is a block</h1>
    }
    return (

        <div>BlogContent</div>
    )
}
